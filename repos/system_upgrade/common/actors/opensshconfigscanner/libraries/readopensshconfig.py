...
def parse_config(config, base_config=None, current_cfg_depth=0):
    ...
    :param Optional[OpenSshConfig] base_config: Base configuration that is extended with configuration options from current file.
            ...
            for included_config_file in files_matching_include_pattern:
                ...
                if parse_config(output, base_config=ret, current_cfg_depth=current_cfg_depth + 1) is None:
                    raise StopActorExecutionError(
                        ...
                        details={'details': 'Too many recursive includes while parsing {}.'.format(included_config_file)}
                    )
